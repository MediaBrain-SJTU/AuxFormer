...
def main():
    ...
    for epoch in range(0, args.epochs):
        ...
        if epoch % args.test_interval == 0:
            ...
            if avg_avg_mpjpe < best_test_loss:
                ...
                if args.model_save_name == 'default':
                    if args.future_length == 30 or args.future_length == 10:
                        file_path = os.path.join(args.model_save_dir, '3dpw_ckpt_long_best.pth.tar')
                    else:
                        file_path = os.path.join(args.model_save_dir, '3dpw_ckpt_best.pth.tar')
                else:
                    if args.future_length == 30 or args.future_length == 10:
                        file_path = os.path.join(args.model_save_dir, args.model_save_name+'3dpw_ckpt_long_best.pth.tar')
                    else:
                        file_path = os.path.join(args.model_save_dir, args.model_save_name+'3dpw_ckpt_best.pth.tar')
            ...
            if args.model_save_name == 'default':
                if args.future_length == 30 or args.future_length == 10:
                    file_path = os.path.join(args.model_save_dir, '3dpw_ckpt_long_'+str(epoch)+'.pth.tar')
                else:
                    file_path = os.path.join(args.model_save_dir, '3dpw_ckpt_'+str(epoch)+'.pth.tar')
            else:
                if args.future_length == 30 or args.future_length == 10:
                    file_path = os.path.join(args.model_save_dir, args.model_save_name+'3dpw_ckpt_long_'+str(epoch)+'.pth.tar')
                else:
                    file_path = os.path.join(args.model_save_dir, args.model_save_name+'3dpw_ckpt_'+str(epoch)+'.pth.tar')
            torch.save(state, file_path)
