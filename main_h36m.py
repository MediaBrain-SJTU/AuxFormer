    ...
    for epoch in range(0, args.epochs):
        ...
        if epoch % args.test_interval == 0:
            if args.future_length == 25 or args.future_length == 13:
                ...
                avg_mpjpe_no_posing = np.zeros((6))
            elif args.future_length == 15:
                avg_mpjpe = np.zeros((2))
                avg_mpjpe_no_posing = np.zeros((2))
            else:
                avg_mpjpe = np.zeros((4))
                avg_mpjpe_no_posing = np.zeros((4))
            ...
            avg_mpjpe_no_posing = avg_mpjpe_no_posing / (len(acts)-1)
            ...
            if avg_avg_mpjpe < best_test_loss:
                ...
                if args.model_save_name == 'default':
                    if args.future_length == 25 or args.future_length == 13:
                        file_path = os.path.join(args.model_save_dir, 'h36m_ckpt_long_best.pth.tar')
                    else:
                        file_path = os.path.join(args.model_save_dir, 'h36m_ckpt_best.pth.tar')
                else:
                    if args.future_length == 25 or args.future_length == 13:
                        file_path = os.path.join(args.model_save_dir, args.model_save_name+'_h36m_ckpt_long_best.pth.tar')
                    else:
                        file_path = os.path.join(args.model_save_dir, args.model_save_name+'_h36m_ckpt_best.pth.tar')
            ...
            if args.model_save_name == 'default':
                if args.future_length == 25 or args.future_length == 13:
                    file_path = os.path.join(args.model_save_dir, 'h36m_ckpt_long_'+str(epoch)+'.pth.tar')
                else:
                    file_path = os.path.join(args.model_save_dir, 'h36m_ckpt_'+str(epoch)+'.pth.tar')
            else:
                if args.future_length == 25 or args.future_length == 13:
                    file_path = os.path.join(args.model_save_dir, args.model_save_name+'_h36m_ckpt_long_'+str(epoch)+'.pth.tar')
                else:
                    file_path = os.path.join(args.model_save_dir, args.model_save_name+'_h36m_ckpt_'+str(epoch)+'.pth.tar')
            torch.save(state, file_path)
